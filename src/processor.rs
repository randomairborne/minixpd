    ...
    if let Some(data) = interaction.data {
        let resp = match data {
            ...
            InteractionData::MessageComponent(mc) => {
                let parent = interaction.message.ok_or(Error::NoParentMessage)?;
                crate::leaderboard::process_message_component(mc, parent, guild_id, state).await?
            }
            _ => PONG,
        };
        Ok(resp)
...
async fn process_slash_cmd(
    ...
    match data.name.as_str() {
        ...
        "leaderboard" => {
            ...
            crate::leaderboard::leaderboard(guild_id, state.db, prefs).await
        }
        "toy" => {
